...
class ParticipantsModel {
	...
	createParticipant(name, email, phone, address, birthdate, category, gender) {
		return new Promise(async (resolve, reject) => {
			try {
				...
				(await this.db).query(
					`insert into participants (name, email, phone, address, birthdate, category, gender) VALUES ('${name}', '${email}', '${phone}', '${address}', '${birthdate}', '${category}', '${gender}') RETURNING id`,
					(err, response) => {
	...
	udpateParticipant(id, propsToEdit) {
		...
		return new Promise(async (resolve, reject) => {
			try {
				(await this.db).query(
					`update participants SET ${propsToUpdate} where id=${id};`,
					(err, response) => {
						// console.log(response)
						let insertResult = response.rowCount;
		...
		return new Promise(async (resolve, reject) => {
			...
			try {
				...
				const participantsQuery = `
                    SELECT participantid, email, name, phone, orderid, paymentdetails, payment_status
                    FROM participants 
                ...
                `;
				const customValuesResult = await (
					await this.db
				...
				const participantsWithCustomValues = participants.map((participant) => {
					const participantCustomValues = customValues.filter(
						(customValue) =>
							customValue.participantid === participant.participantid
					);
	...
	getParticipantsByEvent(eventid) {
		return new Promise(async (resolve, reject) => {
			...
			try {
				...
				(await this.db).query(
					`SELECT participantid,birthdate, email, name, category, phone, address, gender, orderid, paymentdetails from  participants inner join tickets on participants.id = tickets.participantid inner join orders o on o.id = tickets.orderid where eventid=${eventid};`,
					(err, response) => {
						// (await this.db).query(`
