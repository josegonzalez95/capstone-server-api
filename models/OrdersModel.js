...
class OrdersModel {
	constructor() {
		...
		const connection_url =
			'postgres://qlxouxhpuqlcli:c416400a0bd65ef07cc531dbe05b05e643983c24c7019898e083bdffc214a672@ec2-23-20-211-19.compute-1.amazonaws.com:5432/d7mu35vh781rtv';
	...
	createOrder(orderEmail, orderDetails) {
		return new Promise(async (resolve, reject) => {
			try {
				...
				(await this.db).query(
					`INSERT INTO orders (orderemail, paymentdetails) VALUES ('${orderEmail}', '${orderDetails}') RETURNING id`,
					(err, response) => {
	...
	readEventOrders(eventId) {
		return new Promise(async (resolve, reject) => {
			try {
				// const db = await this.pool.connect()
				(await this.db).query(
					`SELECT distinct orderid, paymentdetails, payment_status, date_created, amount_payed, service_fee, transaction_fee FROM orders inner join tickets t on orders.id = t.orderid WHERE eventid=${eventId};`,
					(err, response) => {
	...
	orderParticipants(orderId) {
		return new Promise(async (resolve, reject) => {
			...
			try {
				...
				(await this.db).query(
					`SELECT orderid, paymentdetails, payment_status, date_created, amount_payed, eventid, name, email, phone, category
	...
	udpateOrder(id, propsToEdit) {
		...
		return new Promise(async (resolve, reject) => {
			try {
				// const db = await this.pool.connect()
				(await this.db).query(
					`update orders SET ${propsToUpdate} where id=${id};`,
					(err, response) => {
						// console.log(response)
						let insertResult = response.rowCount;
