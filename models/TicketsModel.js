...
class TicketsModel {
	...
	createTicket(orderid, participantid, eventid) {
		return new Promise(async (resolve, reject) => {
			try {
				...
				(await this.db).query(
					`INSERT INTO tickets (participantid, orderid,eventid) VALUES (${participantid}, ${orderid}, ${eventid})`,
					(err, response) => {
	...
	updateTicket(id, propsToEdit) {
		...
		return new Promise(async (resolve, reject) => {
			try {
				// const db = await this.pool.connect()
				(await this.db).query(
					`update tickets SET ${propsToUpdate} where id=${id};`,
					(err, response) => {
	...
	numberOfTickets(ticketid) {
		return new Promise(async (resolve, reject) => {
			try {
				// const db = await this.pool.connect()
				(await this.db).query(
					`select count(id) from (select * from tickets where eventid =${ticketid}) as total`,
					(err, response) => {
						let result = response.rows[0];
						return resolve({
							result: result,
	...
	async numOfParticipantsPerPromoter(promoterid, res) {
		try {
			(await this.db).query(
				`select count(promoterid) from (select * from promoters inner join public.events e on promoters.id = e.promoterid
                inner join public.tickets t on e.id = t.eventid  where promoterid=${promoterid}) as foo`,
				(err, response) => {
					let result = response.rows[0];
