...
class TicketsController {
	...
	insertTicket(orderID, participantID, eventID) {
		return new Promise(async (resolve, reject) => {
			try {
				const newTicket = await this.model.createTicket(
					orderID,
					participantID,
					eventID
				);
	...
	editTicket(id, participantid, orderid, eventid) {
		return new Promise(async (resolve, reject) => {
			try {
				const propsToEdit = [
					{ value: participantid, propName: 'participantid' },
					{ value: orderid, propName: 'orderid' },
					{ value: eventid, propName: 'eventid' },
				];
	...
	async sumTicketPerPromoter(req, res) {
		try {
			const { promoterid } = req.body;
			await this.model.numOfParticipantsPerPromoter(promoterid, res);
		} catch (error) {
			console.log(error);
		}
	}
